...
- **Step 3E-1 drafted locally**: Team foundation + read routes (`GET /api/team`, `GET /api/team/[id]`) drafted on 2026-05-05 with new `src/lib/team-constants.ts` file. Implements approved decisions A1 (full firm team list including email visible to ARTICLE_STAFF), B1 (inactive hidden by default; `?status=active|inactive|all`), C1 (`q` search is name-only, case-insensitive substring match), D1 (`team-constants.ts` created with `FIRM_ROLES`, `TEAM_STATUS_FILTERS`, `DEFAULT_TEAM_PAGE_SIZE = 50`, `MAX_TEAM_PAGE_SIZE = 200`, `DEFAULT_TEAM_STATUS_FILTER = "active"`). Response shape: `firmMemberId`, `userId`, `name`, `email`, `firmRole`, `isActive`, `joinedAt`. Excluded from response: `passwordHash`, `platformRole`, `PlatformUser.isActive`, `lastLoginAt`, `firmId`. `[id]` is `FirmMember.id` (not `PlatformUser.id`). Cross-firm 404 + `console.warn` on `/api/team/[id]` per Section 25.4 #15. PLATFORM_OWNER without firmId returns 400. Files shipped: `src/lib/team-constants.ts` (NEW, ~40 lines), `src/app/api/team/route.ts` (NEW, ~135 lines: GET only), `src/app/api/team/[id]/route.ts` (NEW, ~95 lines: GET only). No POST / PATCH / deactivate / reactivate (deferred to 3E-2). No `src/lib/permissions.ts` change (matrix already fixed in `96c57db`). No schema, package, or config change. Pending Pankaj's `npm run uat:check` build verification and commit/push approval. Latest verified runtime/code commit (line above) NOT advanced — stays at `8bcf4d1` until this implementation commit pushes and Netlify verifies. With 3E-1 in place, only 3E-2 (team mutations) and 3F (modules) remain pending in Section 14 Step 3. See C-2026-05-05-02 and D-2026-05-05-02.
...
- Step 3 (API layer scaffold): **PARTIALLY DONE**. Sub-steps 3A (commit `093a816`), 3B (commit `d1fad2f`), 3C (commit `7e62c99`), 3D-1 (Tasks foundation + read/create routes, C-2026-05-04-01, commit `8754760`), 3D-2 (Tasks mutations: PATCH + notes + assignees, C-2026-05-04-03 / D-2026-05-04-02, commit `13d8b4f`), and 3D-3 (Tasks lifecycle actions: close + reopen + cancel, C-2026-05-04-05 / D-2026-05-04-03, commit `8bcf4d1`) **DONE** — pushed, deployed, and Netlify-verified. Full Section 14 Step 3D (Tasks route group) is now complete. Sub-step 3E-1 (Team foundation + read routes, C-2026-05-05-02 / D-2026-05-05-02) **drafted locally** pending Pankaj's `npm run uat:check` build verification and commit/push. Origin's 5 firm / tenant routes still present, untouched. Pending: 3E-2 team mutations, 3F modules.
