...
3. **API layer scaffold** - **PARTIALLY DONE**. Cloud Codex shipped 5 origin API routes: `src/app/api/firms/`, `src/app/api/firms/[firmId]/`, `src/app/api/firms/[firmId]/access/`, `src/app/api/firms/[firmId]/members/`, `src/app/api/tenant/validate/`. Sub-step 3A (permissions map + API helper foundation, commit `093a816`), 3B (clients routes + soft-delete, commit `d1fad2f`), and 3C (activity read route, commit `7e62c99`) **DONE**. Sub-step 3D-1 (Tasks foundation + read/create routes, C-2026-05-04-01 / D-2026-05-04-01) **DRAFTED LOCALLY** pending Pankaj's `npm run uat:check` build verification and commit/push. Pending sub-waves: 3D-2 (Tasks mutations), 3D-3 (Tasks lifecycle actions). Pending route groups: `team/`, `modules/`.
