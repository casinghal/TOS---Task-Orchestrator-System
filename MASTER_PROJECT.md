...
5. **Persistence cutover** - **IN PROGRESS (decomposed into 5A + 5B)**.
   ...
   - **5B (persistence / data cutover): NOT STARTED — next controlled sub-step**. Wire the UI to consume the API (thin typed data-access layer; incremental entity-by-entity), add the one-time browser-side export endpoint, activate `ActivityLog` writes from the UI side, full validation pass against Postgres, then remove the localStorage source-of-truth code. Use Opus 4.7 for 5B plan-first (one-way migration risk). RLS remains a separate pre-pilot security wave, not part of Step 5.
