...
**Foundation status (2026-06-15, Gate 1-A1a DONE; CHANGE_LOG C-2026-06-15-02, DECISION_LOG D-2026-06-15-01).** The enforcement substrate now exists and was validated PASS. Created via the Supabase SQL editor: a constrained role `practiceiq_app` (LOGIN, NOBYPASSRLS, non-superuser / non-createdb / non-createrole, zero table grants) and an `app` schema with bootstrap helpers (`resolve_session(text)` and `begin_impersonation(...)` SECURITY DEFINER owned by `postgres`, PUBLIC execute revoked; plus the GUC accessors `current_firm_id` / `current_actor` / `is_platform_owner`). The constrained role authenticates through the Supabase pooler (R1 resolved). **Nothing is enforced yet:** the app still connects as `postgres`, RLS is not enabled, and no policies or table grants exist. The 1-A1a SQL was applied manually and is not yet a committed migration. Remaining sequence: 1-B0 (plan-only) tenant-context plumbing (`withTenant` + per-transaction `set_config`), then later gates for table grants, RLS + policies, and the connection-role flip (1-D, the real enforcement and production-release event).
