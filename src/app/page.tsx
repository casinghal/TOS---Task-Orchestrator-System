...
// Practice Intelligence v0: deterministic, frontend-only assist layer. No AI/LLM, no
// new API/network calls, no localStorage source-of-truth. Renders a dashboard preview
// card from already-loaded data plus point-of-work suggestion chips that only prefill
// existing inputs (the user still submits). Flip to false to hide the v0 layer.
const PRACTICE_INTELLIGENCE_V0_ENABLED = true;
const PI_TASK_TEMPLATES = ["Monthly close follow-up", "Client document pending", "Review blocker", "Internal QC check", "Manager review required"];
const PI_STATUS_SNIPPETS: { label: string; text: string }[] = [
  { label: "Partial data received", text: "Partial data has been received. Bank statement is still pending from the client." },
  { label: "In progress", text: "Work is in progress. Review is expected by the planned date." },
  { label: "Pending clarification", text: "Pending client clarification before the next step can be completed." },
  { label: "Submitted for review", text: "Submitted for manager review." },
  { label: "Ready for closure", text: "Review comments have been addressed and the task is ready for closure." },
];
...
function RoleDashboardView({ assignments, clients, modules, openAssignment, openTask, setActive, tasks, team, user }: { assignments: Assignment[]; clients: Client[]; modules: ModuleFlag[]; openAssignment: () => void; openTask: (id: string) => void; setActive: (section: Section) => void; tasks: Task[]; team: TeamMember[]; user: TeamMember }) {
  ...
  const piSoonIso = new Date(`${todayIso}T00:00:00.000Z`);
  piSoonIso.setUTCDate(piSoonIso.getUTCDate() + 7);
  const piSoonIsoString = piSoonIso.toISOString().slice(0, 10);
  const piDueSoon = tasks.filter((task) => task.status !== "Closed" && task.dueDate >= todayIso && task.dueDate <= piSoonIsoString).length;
  const piSteps = [
    { label: "Add your first client", done: clients.length > 0 },
    { label: "Create your first task", done: tasks.length > 0 },
    { label: "Move a task into review", done: tasks.some((task) => task.status === "Under Review") || closedCount > 0 },
    { label: "Close a reviewed task", done: closedCount > 0 },
  ];
  const piNextStep = piSteps.find((step) => !step.done);
  ...
  return <div className="space-y-5">
    ...
    {PRACTICE_INTELLIGENCE_V0_ENABLED && <section className="rounded-xl border border-blue-200 bg-blue-50/40 p-4 shadow-sm md:p-5">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h3 className="text-base font-semibold text-slate-950">Practice Intelligence — Preview</h3>
          <p className="mt-1 text-sm text-slate-600">Rule-based suggestions from your current data. No external AI. Early preview.</p>
        </div>
        <span className="rounded-full border border-blue-200 bg-white px-2.5 py-1 text-xs font-medium text-blue-700">Smart suggestions</span>
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-white p-3">
          <p className="text-sm font-semibold text-slate-800">What needs attention</p>
          <ul className="mt-2 space-y-1 text-sm text-slate-600">
            <li>{overdue} task{overdue === 1 ? "" : "s"} overdue</li>
            <li>{piDueSoon} due in the next 7 days</li>
            <li>{underReview} waiting under review</li>
            <li>{openCount} open across {clients.length} client{clients.length === 1 ? "" : "s"}</li>
          </ul>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-3">
          <p className="text-sm font-semibold text-slate-800">Recommended next steps</p>
          <ul className="mt-2 space-y-1.5 text-sm">
            {piSteps.map((step) => <li key={step.label} className="flex items-center gap-2">{step.done ? <CheckCircle2 size={16} className="text-emerald-600" /> : <span className="inline-block h-4 w-4 rounded-full border border-slate-300" />}<span className={step.done ? "text-slate-400 line-through" : "text-slate-700"}>{step.label}</span></li>)}
          </ul>
          {piNextStep && <p className="mt-3 text-xs text-slate-500">Suggested focus: {piNextStep.label}.</p>}
        </div>
      </div>
    </section>}
...
function TaskModal({ clients, close, submit, team }: { clients: Client[]; close: () => void; submit: (values: { title: string; clientId: string; dueDate: string; priority: string; assigneeIds: string[]; reviewerId: string; description?: string }) => Promise<{ ok: boolean; message?: string }>; team: TeamMember[] }) {
  ...
  function applyTemplate(value: string) {
    const el = formRef.current?.elements.namedItem("title") as HTMLInputElement | null;
